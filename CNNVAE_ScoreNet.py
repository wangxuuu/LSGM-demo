...
vae = vae_models[config['model_params']['name']](in_channels=3, latent_dim=config['model_params']['latent_dim'], loss_type=config['model_params']['loss_type']).to(device)
...
for epoch in range(config['trainer_params']['max_epochs']):
    for i, (x, y) in enumerate(data_loader):
        ...
        if config['exp_params']['joint_train']:
            ...
            vae_losses = vae.loss_function(x_reconst, x, mu, log_var, config['exp_params']['kld_weight'])
